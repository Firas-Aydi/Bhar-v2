...
exports.getOneBookDetailsController = (req, res, next) => {
  ...
  bookModel.getOneBookDetails(id).then((resbook) => {
    res.render("details", {book:resbook,verifUser: req.session.userId,verifType: req.session.type});
  });
};

exports.getAddBookController = (req, res, next) => {
  res.render("addbook", { verifUser: req.session.userId,Smessage:req.flash('Successmessage')[0],Emessage:req.flash('Errormessage')[0],verifType: req.session.type });
};
